...
void GraphicsNode::UseMesh(std::function<void(RenderMesh&)> context) { std::lock_guard lock(meshMutex); context(mesh); }
void GraphicsNode::UseMeshConst(std::function<void(const RenderMesh&)> context) { std::lock_guard lock(meshMutex); context(mesh); }
